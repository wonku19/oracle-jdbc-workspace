...
-- DEPT_COPY 테이블에 CNAME 컬럼 추가하기
ALTER TABLE DEPT_COPY ADD CNAME VARCHAR2(20);
-->> 새로운 컬럼이 만들어지고 기본값으로 NULL이 채워짐

-- DEPT_COPY 테이블에 LNAME 컬럼 추가하기 (기본값을 지정한 채로)
ALTER TABLE DEPT_COPY ADD LNAME VARCHAR2(20) DEFAULT '한국';
-->> 새로운 컬럼이 만들어지고 내가 지정한 '한국' 이 기본값으로 채워짐
...
/*
1-2) 컬럼 수정 (MODIFY)
- 데이터 타입 변경 : ALTER TABLE 테이블명 MODFIY 컬럼명 변경할 데이터타입;
- 기본값 변경 : ALTER TABLE 테이블명 MODIFY 컬럼명 DEFAULT 변경할 기본값;
              --> DEFAULT NULL : DEFAULT 삭제
*/

-- DEPT_ID 컬럼의 데이터 타입을 CHAR(3) 으로 변경하기
ALTER TABLE DEPT_COPY MODIFY DEPT_ID CHAR(3);
ALTER TABLE DEPT_COPY MODIFY DEPT_ID NUMBER; -- 변경할 DEPT_ID 에 이미 NUMBER 과 맞지 않는 값이 있어서 실행 시 오류 발생됨
ALTER TABLE DEPT_COPY MODIFY DEPT_TITLE VARCHAR2(10); -- 변경할 DEPT_TITLE 에 변경하는 자료형의 크기보다 이미 큰 값이 존재해서 실행 시 오류 발생됨

ALTER TABLE DEPT_COPY MODIFY CNAME NUMBER;
--> 값이 없으면 데이터타입 변경 가능

-- 다중 수정
-- DEPT_COPY 테이블에서 DEPT_TITLE 컬럼의 데이터 타입을 VARCHAR2(40)으로
-- LOCATION_ID 컬럼의 데이터 타입을 VARCHAR2(2) 으로
-- LNAME 컬럼의 기본값을 미국으로 변경하기

ALTER TABLE DEPT_COPY MODIFY DEPT_TITLE VARCHAR2(40) 
                      MODIFY LOCATION_ID VARCHAR2(2) 
                      MODIFY LNAME DEFAULY '미국';

/*
1_3) 컬럼 삭제
ALTER TABLE 테이블명 DROP COLUMN 컬럼명;
- 데이터 값이 기록되어 있어도 같이 삭제된다 (삭제된 컬럼 복구는 불가능 ROLLBACK도 안됨)
- 참조되고 잇는 컬럼이 있다면 삭제 불가능
...
ROLLBACK; -- ROLLBACK 진행해도 삭제된 테이블 복구 안됨
...
*/
-- DEPT_COPY 테이블의 LNAME 컬럼명을 LOCATION_NAME 으로 변경하기
...
-- DEPT_ COPY 테이블의 DEPT_ID 컬럼에 PK 제약조건 추가하기
...
*/
-- DEPT_COPY 테이블의 제약조건명 DEPT_COPY_DEPT_ID_PK 인 제약조건 삭제하기
...
*/
-- DEPT_COPY 테이블의 제약조건명 DEPT_COPY_DEPT_ID_PK 변경해보기
...
위 2가지 중 한가지로 테이블명 변경 가능
*/
-- DEPT_COPY 를 DEPT_TEST로 변경하기
...
-- 만약 부모 테이블을 삭제하고자 한다면 
-- 방법 1. 자식테이블을 먼저 삭제한 후 부모 테이블을 삭제하는 방법
DROP TABLE MEMBER;
DROP TABLE MEMBER_GRADE;
-- 방법 2. 그냥 부모테이블만 삭제하는데 제약조건까지 같이 삭제하는 방법
DROP TABLE MEMBER_GRADE CASCADE CONSTRAINT;
-->> 강제 삭제이기 때문에 가능하면 방법1로 삭제 진행하길 권장함
...
CREATE TABLE MEMBER (
    ...
    GENDER CHAR(3),
    AGE NUMBER,
    GRADE_ID NUMBER CONSTRAINT MEMBER_GRADE_ID_FK REFERENCES MEMBER_GRADE
    );