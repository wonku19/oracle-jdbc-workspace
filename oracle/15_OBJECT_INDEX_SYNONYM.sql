...
-- 현재 계정 소유의 인덱스 정보를 열람하고 싶을때
SELECT *
FROM USER_INDEXES;
SELECT *
FROM USER_IND_COLUMNS;

-- 인덱스가 걸린 컬럼으로 데이터를 조회
-- 데이터가 얼마 없으면 오라클에서 굳이 인덱스를 사용해서 조회하지 않는다  
SELECT *
FROM EMPLOYEE
WHERE EMP_ID = 210;
...
- 인덱스가 항상 좋은 결과로 이어지지는 않는다
- 정확한 데이터 분석에 기반을 두지 않은 무분별한 인덱스는 오히려 성능을 떨어뜨린다
*/
...
-- 1. 비공개 SYNONYM 생성
-- 권한을 부여해야만 동의어 생성 가능
-- 관리자 계정(sys)로 kh 계정에 SYNONYM 생성 권한을 준다.
GRANT CREATE SYNONYM TO KH;

-- EMPLOYEE 테이블에 비공개 SYNONYM 생성 (KH 계정에서 / PUBLIC 을 쓰지 않으면 비공개)
