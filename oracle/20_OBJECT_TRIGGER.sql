 ...
 EX) 
 - 회원 탈퇴 시 기존의 회원테이블에 데이터 DELETE 후 곧바로 탈퇴된 회원들만 따로 보관하는 테이블에 자동으로 INSERT 처리
...
- SQL문에 의해 영향을 받는 각 행에 따른 분류
   ...
   > ROW TRIGGER(행 트리거) : 해당 SQL문 실행할 때마다 매번 트리거를 실행 (FOR EACH ROW 옵션 기술해야함)
...
BEFORE|ATRER INSERT|UPDATE|DELETE ON 테이블명
[FOR EACH ROW]
DECLARE
    변수선언;
BEGIN
...
-- EMPLOYEE 테이블에 UPDATE 수행 후 메시지를 자동으로 출력하기
